...
export function renderOrderSummary() {
    ...
    cartModule.cart.forEach((cartItem) => {
        ...
        const today = dayjs();
        const deliverDate = today.add(deliveryOption.deliveryDays, 'days').format('dddd, MMMM, D');
    ...
    `;
        ...
        function deliveryOptionsHTML(matchingProduct, cartItem) {
            ...
            deliveryOptions.forEach((deliveryOption) => {
                const todays = dayjs();
                const deliverDate = today.add(deliveryOption.deliveryDays, 'days').format('dddd, MMMM, D');
                const priceString = deliveryOption.price === 0 ? 'FREE' : `$${formatCurrency(deliveryOption.price)} - Shipping`;
